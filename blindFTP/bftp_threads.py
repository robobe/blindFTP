...
# 22/02/2006 v0.13 LV: - optimisation des synchro (v�rification que les fichiers �
#                        �mettre sont stables)
...
#                      - Sauvegarde de l'�tat de transmission dans un fichier XML
#                      - Option mode reprise
...
#------------------------------------------------------------------------------
# A FAIRE:
# - am�liorer le multithreading (lock sur fichier et non sur chaque paquet)
# - corriger affichage de l'aide (UnicodeError)
# - mettre les extensions de fichiers � exclure dans une liste param�trable
# - fichier de config style .ini
# - reprendre la gestion des num�ros de session avec l'ordonnanceur de synchro_arbo
...
TAILLE_PAQUET = 65500   # taille max d'un paquet r�seau
...
# Ent�te d'un paquet BFTP (format v4):
...
#    le taux de perte constat� empiriquement (trop vite = des paquets perdus,
#    trop lentement = faible r�activit� et parcours de l'arborescence trop long)
#
# Redondance d'�mission des fichiers
MinFileRedundancy=5
    ...
    def traiter_paquet(self, paquet):
        ...
        if not self.paquets_recus.get (paquet.num_paquet):
            ...
            if self.paquets_recus.nb_true == self.nb_paquets:
                # on va � la ligne
                #print ""
                # Mise en thead de la recopie afin de liberer de la ressource pour la r�ception
                recopie=threading.Thread(None, self.recopier_destination, None,())
                recopie.start()
                # TODO : reporter les affichages ci-dessous au sein de la fonction mise en thread et "ignorer" les donn�es recu jusqu'� fin du thread
...
# creuser le fonctionnement en thread : le programme de r�ception en thread semble moins performant
# que dans la version sans thread
# sur une arborescence de 20 fichiers � la premier it�ration :
...
class BlindFTPServer(ThreadingUDPServer, InterruptibleServer):
    ...
    def finish_request(self, (paquet, sock), emetteur):
        """
        Traitement d'un datagramme UDP: Chaque paquet est trait� par un thread
        s�par�.
        """
        ...
        try:
            ...
            # � un moment donn�.
            # (pour �viter l'acc�s simultan� � un m�me fichier et � l'objet p)
            with lock_paquet:
...
def synchro_arbo(repertoire):
    ...
    if (0):
        ...
        # Traitement distant des donnn�es :
            ...
            for f in sorted(only2, reverse=True):
                ...
                if(DRef.dict[f].tag == xfl.TAG_FILE):
                    ...
                    if (time.time() - (LastView + OffLineDelay)) > 0:
                        # Disparu depuis X jours ; le fichier a �t� effac�
...
def analyse_options():
    ...
    parseur.add_option("-c", "--continue", action="store_true", dest="reprise",\
        default=False, help="Fichier de reprise � chaud")
